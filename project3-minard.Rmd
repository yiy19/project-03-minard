...
```{r refined-minard, fig.height=5, fig.width=10}
...
tempdateplot <- minard_temp %>% 
  ...
  geom_path() +
  geom_point() +
  geom_text(mapping = aes(label = temp, vjust = 1.6, family = "sans"), size = 4, show.legend = FALSE) + 
  geom_text(mapping = aes(label = date), vjust = 3.6, color = "#495057", size = 3) +
  scale_y_continuous(limits = c(-40, 0), position = "right") +
  scale_x_continuous(breaks = NULL) +
  labs(y = "Temperature & Date",
      x = NULL) +
  xlab(NULL) +
      theme(    
          axis.ticks = element_blank(),       # remove ticks (little lines sticking out of the axis)
...
```{r join-plots}
# join two plots
minard_final <- ggarrange(minard_surv, tempdateplot,
                          ncol = 1,
                          nrow = 2)
minard_final
